...
$(function() {
	$("input:checkbox").click(function() {
		...
		$.ajax({
			...
			url: "http://localhost:8080/exam220402/edu/admin/getChooseSubId",
			success: function(data) {
				if(data.trim().indexOf("f-exists-") != -1) {
					...
					if($("#"+checked)[0].checked) {
						$("#choosed").text(parseInt($("#choosed").text())-1);
					}else{
						$("#choosed").text(parseInt($("#choosed").text())+1);
					}
					//截取题号
	...
	$("#isAddHandle").click(function() {
		...
		$.ajax({
			type: "POST",
			data: "examPaperId="+examPaperId+"&subIds="+subIds,
			url: "http://localhost:8080/exam220402/edu/admin/handAdd",
			success: function(data) {
...
function examPaperAddSubjects(examPaperId) {
	zeroModal.show({
		title : '添加试题-手动',
		iframe : true,
		url : 'http://localhost:8080/exam220402/edu/admin/listSubject?handAdd=1&examPaperId='+examPaperId,
		width : '90%',
		height : '90%',
		cancel : true,
		top : '0px',
		...
		onClosed : function() {
			$("form input").attr("disabled", "disabled");
			$("form").attr("method", "get");
			$("form").attr("action", "listExamPaper").submit();
		}
	});
}
...
function autoAddSubjects(examPaperId) {
	zeroModal.show({
		title : '自动生成试题',
		iframe : true,
		url : 'http://localhost:8080/exam220402/edu/admin/toAutoSubjectSelect?examPaperId='+examPaperId,
		width : '30%',
		height : '80%',
		top : '30px',
		left : '430px',
