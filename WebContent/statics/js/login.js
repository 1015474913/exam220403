...
$(function() {
	...
	$("#inputEmail").blur(function() {
		...
		$.post("exam220402/admin/teacherLogin", "account="+account, function(data) {
			if(data == "1") {
				$("#inputEmail").val("");
	...
	$("#studentLogin").click(function() {
		zeroModal.show({
			title : '登 录',
			iframe : true,
			url : '/exam220402/reception/tologin',
			width : '30%',
			height : '50%',
			top : '100px',
			left : '430px',
	...
	$("#inputEmail3").blur(function() {
		...
		$.post("/exam220402/reception/checkStuAccount", "studentAccount="+studnetAccount, function(data) {
			console.log(data);
			if(data == "n") {
				$("#inputEmail3").val("");
	...
	$("#studentRegister").click(function() {
		...
		zeroModal.show({
			title : '注册',
			iframe : true,
			url : '/exam220402/reception/toaddStudent',
			width : '28%',
			height : '45%',
			top : '100px',
			left : '430px',
	...
	$("#account").blur(function() {
		...
		/*$.ajax({
			...
			url: "/exam220402/reception/addStudent.json",
	...
	$("#signSubmit").click(function() {
		...
		var studentCalssId =$("#calssId").val();
		...
		/*$.ajax({
			type: "POST",
			url: "/exam220402/reception/addStudent",
			...
			success: function(data) {
				if (data.trim() == "t") {
					...
					setTimeout(function(){
						window.location.href = '/exam220402/reception/login';
					},1000);
				} else {
					alert("注册失败");
				}
