...
<body>
	...
	<div style="text-align: center;">
			...
			<thead>
				<tr>
					<c:if test="${handAdd != null }">
						<th>
							已选:
							<span id="choosed" style="color: red;">0</span>
						</th>
					</c:if>
					<th>试题编号</th>
					<th>题目</th>
			...
			<tbody>
				<c:choose>
					<c:when test="${not empty subjects }">
						<c:forEach items="${subjects.list }"   var="subject">
							<tr id="tr-${subject.subjectId }">
								...
								<c:if test="${fn:length(subject.subjectName) > 8 }">
										...
										<a href="#"   onclick="_iframe(1,' ${path }/edu/subject/toSubjectTest?subjectId=${subject.subjectId }', 'listSubject')">[...]</a>
									</c:if>
									<c:if test="${fn:length(subject.subjectName) <= 8 }">
										${subject.subjectName }
									 </c:if> 
								...
								<td>
									<c:if test="${fn:length(subject.optionA) > 5 }"> ${fn:substring(subject.optionA, 0, 5) } <sub><a href="#" onclick="_iframe(1,'${path }/edu/subject/toSubjectTest?subjectId=${subject.subjectId }', 'listSubject')">[...]</a></sub> </c:if>
									<c:if test="${fn:length(subject.optionA) <= 5 }"> ${subject.optionA }</c:if>
								</td>
								<td>
									<c:if test="${fn:length(subject.optionB) > 5 }"> ${fn:substring(subject.optionB, 0, 5) } <sub><a href="#" onclick="_iframe(1,'${path }/edu/subject/toSubjectTest?subjectId=${subject.subjectId }', 'listSubject')">[...]</a></sub> </c:if>
									<c:if test="${fn:length(subject.optionB) <= 5 }"> ${subject.optionB }</c:if>
								</td>
								<td>
									<c:if test="${fn:length(subject.optionC) > 5 }"> ${fn:substring(subject.optionC, 0, 5) } <sub><a href="#" onclick="_iframe(1,'${path }/edu/subject/toSubjectTest?subjectId=${subject.subjectId }', 'listSubject')">[...]</a></sub> </c:if>
									<c:if test="${fn:length(subject.optionC) <= 5 }"> ${subject.optionC }</c:if>
								</td>
								<td>
									<c:if test="${fn:length(subject.optionD) > 5 }"> ${fn:substring(subject.optionD, 0, 5) } <sub><a href="#" onclick="_iframe(1,' ${path }/edu/subject/toSubjectTest?subjectId=${subject.subjectId }', 'listSubject')">[...]</a></sub> </c:if>
								...
								<td>
									<c:if test="${handAdd == null }">
										<div class="btn-group">
											<button type="button" class="btn btn-info btn-sm" onclick="_iframe(1,'${path }/edu/subject/toSubjectTest?subjectId=${subject.subjectId }','listSubject')">修改</button>
		...
		<div>
			<ul class="pagination">
				...
				<li><a href="${path }/edu/subject/listSubject?startPage=1&handAdd=${handAdd }&examPaperId=${examPaperId }">首页</a></li>
				<c:if test="${pageNow-1 > 0 }">
					<li><a href="${path }/edu/subject/listSubject?startPage=${pageNow-1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">上一页</a></li>
				</c:if>
				<c:forEach begin="${pageNow }" end="${pageNow+4 }" var="subPage">
					<c:if test="${subPage-5 > 0 }">
						<li><a href="${path }/edu/subject/listSubject?startPage=${subPage-5 }&handAdd=${handAdd }&examPaperId=${examPaperId }">${subPage-5 }</a></li>
					</c:if>
				</c:forEach>
				<c:forEach begin="${pageNow }" end="${pageNow+5 }" step="1" var="pageNo">
					...
					<c:if test="${pageNo <= pageTotal }">
						<c:if test="${pageNow == pageNo }">
							<li class="active"><a href="${path }/edu/subject/listSubject?startPage=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }">${pageNo }</a></li>
						</c:if>
						<c:if test="${pageNow != pageNo }">
							<li><a href="${path }/edu/subject/listSubject?startPage=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }" class="pageLink">${pageNo }</a></li>
						</c:if>
					</c:if>
				</c:forEach>
				<c:if test="${pageNow+1 <= pageTotal }">
					<li><a href="${path }/edu/subject/listSubject?startPage=${pageNow+1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">下一页</a></li>
				</c:if>
				<li><a href="${path }/edu/subject/listSubject?startPage=${pageTotal }&handAdd=${handAdd }&examPaperId=${examPaperId }">尾页</a></li>
				<li>
					<a>${pageNow }/${pageTotal }</a>
				</li>
				<li>
					<div style="width:-1%; height:100%;float:right;">
						<form action="${path }/edu/subject/listSubject"   id="scannerPageForm">
    ...
    <script src="${path }/statics/js/kindeditor/kindeditor-min.js"></script>
   	...
   	<script type="text/javascript">
   		function subjectAdd() {
   			zeroModal.show({
   				title : "试题编辑",
   				iframe : true,
   				url : "${path }/edu/subject/preAddSubject",
   				width : '90%',
   				height : '90%',
   				cancel : true,
   				top : '0px',
	            ...
	            buttons: [{
	                ...
	                fn: function(opt) {
	                	...
	                	$.ajax({
	                		type : "POST",
	                		data : $(doc).serialize(),
	                		url : "${path }/edu/subject/addSubject.json",
	                		dataType:"json",
	                		success : function() {
			                	zeroModal.show({
			        				title : "操作成功",
   		...
   		$(function() {
  			...
  			$(".delSubject").click(function() {
  				...
  				$.ajax({
  					...
  					url: "${path }/edu/subject/delSubject.json",
  					dataType:"json",
  					success: function(data) {
  						...
  						if(data.subjectId == 't') {
  							alert("删除成功!");
  							$("#tr-"+subjectId).remove();
  							//window.location.href="${path }/edu/subject/listSubject";
  							window.location.reload();
  						} /* else {
  							alert("删除失败, 未知异常!");
  						} */
