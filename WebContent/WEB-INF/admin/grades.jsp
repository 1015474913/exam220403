...
<head>
	...
	<c:set var="path" value="<%=basePath %>"></c:set>
 	...
 	<link href="${path }/statics/css/public.css" rel="stylesheet" />
</head>
<body>
...
<%--					<c:if test="${sessionScope.adminPower == 1 }">--%>
						<th>操作
							&emsp;
							<a href="${path}/edu/grades/preAddGrade" class="btn btn-xs btn-info">添加</a>
						</th>
<%--					</c:if>--%>
				</tr>
			</thead>
						...
						<c:forEach items="${grades }" var="grade">
							<tr>
								...
								<td>
									<div class="btn-group">
<%--										<c:if test="${power == 1 }">--%>
											<button type="button" class="btn btn-info btn-sm" onclick="update('${path}/edu/grades/updateGrade/${grade.gradeId }')">修改</button>
											<button type="button" class="btn btn-danger btn-sm" onclick="del('${path}/edu/grades/delGrade/${grade.gradeId }')">删除</button>
<%--										</c:if>--%>
