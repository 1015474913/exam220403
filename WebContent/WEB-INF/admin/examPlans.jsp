...
<body>
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>操作
						&emsp;
						<a type="button" onclick="_iframe(0, '${path }/edu/examplan/preAddep?examPlanId=${plan.examPlanId}', 'examPlans')" class="btn btn-xs btn-info" target="right">添加</a>
					</th>
				</tr>
			</thead>
			<tbody>
					...
					<c:when test="${not empty examPlans }">
						<c:forEach items="${examPlans }" var="plan">
							<tr>
								...
								<td>
									<div class="btn-group">
										<button type="button" class="btn btn-info btn-sm" onclick="_iframe(1,'${path }/edu/examplan/preUpdateep?examPlanId=${plan.examPlanId }', 'examPlans')">修改</button>
										<a href="${path }/edu/examplan/delExamPlan?examPlanId=${plan.examPlanId }" class="btn btn-danger btn-sm">移除</a>
									</div>
								</td>
							</tr>
						</c:forEach>
    ...
    <script src="${path }/statics/js/bootstrap/bootstrap.min.js" charset="UTF-8"></script>
    <script src="${path }/statics/js/handle.js"></script>
    <script src="${path }/statics/js/add-update.js" charset="UTF-8"></script>
</body>
</html>