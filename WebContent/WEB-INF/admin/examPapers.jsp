...
<body>
			...
			<thead>
				<tr>
					...
					<th>操作
						...
						<a href="${pageContext.request.contextPath }/edu/admin/toAddExamPaper" class="btn btn-xs btn-info">添加</a>
					</th>
				</tr>
			</thead>
			<tbody>
				<c:choose>
					<c:when test="${not empty examPapers }">
						<c:forEach items="${pageInfo.list }" var="ep">
							<tr class="subject_info">
								...
								<td>
									<div class="btn-group">
										<a href="${pageContext.request.contextPath }/edu/admin/getExamPaperById?examPaperId=${ep.examPaperId }"  class="btn btn-info btn-sm" >修改</a>
										<%-- <button type="button" class="btn btn-info btn-sm" onclick="_iframe(1, 'examPaper/${ep.examPaperId }', 'examPapers')">修改</button> --%>
										<a href="${pageContext.request.contextPath }/edu/admin/deleteExamPaper?examPaperId=${ep.examPaperId }" class="btn btn-danger btn-sm" >删除</a>
			...
			<div>
				<ul class="pagination"> 
					<c:if test="${pageNow-1 > 0 }">
						<li><a href="${pageContext.request.contextPath }/edu/admin/listExamPaper?startPage=${pageNow-1 }">&laquo;</a></li>
					</c:if>
					<c:forEach begin="1" end="${pageTotal }" step="1" var="pageNo">
						<c:if test="${pageNow == pageNo }">
							<li class="active"><a href="${pageContext.request.contextPath }/edu/admin/listExamPaper?startPage=${pageNo }">${pageNo }</a></li>
						</c:if>
						<c:if test="${pageNow != pageNo }">
							<li><a href="${pageContext.request.contextPath }/edu/admin/listExamPaper?startPage=${pageNo }" class="pageLink">${pageNo }</a></li>
						</c:if>
					</c:forEach>
					<c:if test="${pageNow+1 <= pageTotal }">
						<li><a href="${pageContext.request.contextPath }/edu/admin/listExamPaper?startPage=${pageNow+1 }">&raquo;</a></li>
					</c:if>
				</ul>
			</div>
		</c:if>
    ...
    <script src="${path }/statics/js/add-update.js"></script>
   	<script src="${path }/statics/js/view-link.js"></script>
</body>
</html>