			...
			<thead>
				<tr>
					...
					<c:if test="${sessionScope.adminPower == 1 }">
						<th>操作
							&emsp;
							<button type="button" class="btn btn-xs btn-info" onclick="_iframe(0, '${path}/edu/teachers/addteacher', 'teachers')">添加</button>
						</th>
					</c:if>
				</tr>
			</thead>
							...
							<tr>
								...
								<td>
										<div class="btn-group">
											...
											<c:if test="${sessionScope.adminPower == 1}">
												<button type="button" class="btn btn-danger btn-sm" onclick="del('${path}/edu/teachers/del/${teacher.teacherId }')">删除</button>
											</c:if>
										</div>
								</td>
							</tr>
